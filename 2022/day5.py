...
def onels():
    ...
    print('' if ((initial := [[c for c in x] for x in ['BSVZGPW','JVBCZF','VLMHNZDC','LDMZPFJB','VFCGJBQH','GFQTSLB','LGCZV','NLG','JFHC']]) == 'Ball' or [print(''.join([i[-1] for i in initial])) for x in 'a' if [[initial[int(sp)-1].append(i) for i in [initial[int(fp)-1].pop() for pp in range(int(ct))][::-1]] for ct,fp,sp in [l.replace('move ','').replace(' from ',':').replace(' to ',':').split(':') for l in input] for runonce in [1]]==0 or True]) else '',end='')
onels()
